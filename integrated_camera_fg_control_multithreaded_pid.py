# -*- coding: utf-8 -*-
"""
整合式攝影機追蹤與函數產生器控制系統 - 多執行緒版本 + PID 控制
========================================
改進：
1. 使用多執行緒架構提升 FPS
...
6. PID 控制：
   - Straight mode: 控制 orientation → 0° (保持直線)
   - Rotation mode: 控制累積角度 → 90° (精準旋轉)
...
# ========== 攝影機設定 ==========
MODE              = "straight"  # "straight" or "rotation"
CAMERA_INDEX      = 1
CAM_WIDTH         = 640
CAM_HEIGHT        = 480
CAM_FPS_REQ       = 120
RECORD_OUTPUT     = True
WINDOW_TITLE      = "Integrated Camera & FG Control (Multithreaded + PID)"
DISPLAY_SCALE     = 0.6
...
# 靜止檢測閾值
STATIC_THRESHOLD_PX = 2.0
STATIC_SPEED_THRESHOLD = 1.0

# ========== PID 控制參數 ==========
# Straight mode PID (控制 orientation → 0°)
PID_KP_STRAIGHT = 0.02  # 比例增益
PID_KI_STRAIGHT = 0.0  # 積分增益
PID_KD_STRAIGHT = 0.01  # 微分增益

# Rotation mode PID (控制累積角度 → 90°)
PID_KP_ROTATION = 0.015
PID_KI_ROTATION = 0.0005
PID_KD_ROTATION = 0.008

# PID 輸出限制（電壓範圍）
PID_VOLTAGE_MIN = 0.0
PID_VOLTAGE_MAX = 2.0
PID_VOLTAGE_BASE = 1.2  # 基礎電壓

# PID 目標值
PID_SETPOINT_STRAIGHT = 0.0  # straight mode: 目標角度 0°
PID_SETPOINT_ROTATION = 90.0  # rotation mode: 目標累積角度 90°

# PID 啟用設定
ENABLE_PID = True  # 是否啟用 PID 控制
...
class FunctionGeneratorController:
    ...
    def set_voltage(self, voltage):
        ...
        try:
            # 限制電壓範圍
            voltage = np.clip(voltage, PID_VOLTAGE_MIN, PID_VOLTAGE_MAX)
            
            # 只在電壓變化超過閾值時才更新（減少通訊開銷）
            if abs(voltage - self.current_voltage) < 0.01:
                return True
            
            self.inst.write(f'SOUR1:VOLT {voltage:.3f}; SOUR2:VOLT {voltage:.3f}')
    ...
    def switch_mode(self, mode_num):
        ...
        try:
            ...
            # 設定電壓（如果 PID 啟用，使用當前電壓；否則使用配置的電壓）
            if ENABLE_PID and self.current_voltage != config['ch1_volt']:
                # PID 已經在控制電壓，保持當前值
                pass
            else:
                self.inst.write(f'SOUR1:VOLT {config["ch1_volt"]}; SOUR2:VOLT {config["ch2_volt"]}')
                self.current_voltage = config['ch1_volt']
            self.inst.write('*WAI')
...
def calibrate_scale(frame):
    ...
    if grid_scale is not None and 0.05 < grid_scale < 0.5:
        print(f"    ✓ 網格檢測成功")
        return grid_scale
    else:
        print(f"    ✗ 網格檢測失敗或不可靠")
...
# ========== Thread 2: 影像處理執行緒 (含 PID 控制) ==========
def process_thread(frame_queue, result_queue, running, stats, kf, tracker_state, mm_per_px, fg_controller, pid_controller):
    """從佇列取出幀並進行所有影像處理，包含 PID 控制"""
    ...
    # 累積角度 (用於 rotation mode)
    cumulative_angle = 0.0
    last_unwrapped_angle = None
    ...
    while running[0] or not frame_queue.empty():
        ...
        # 計算累積角度 (用於 rotation mode)
        if np.isfinite(ang_for_draw):
            unwrapped_angle = ang_for_draw
            if last_unwrapped_angle is not None:
                angle_diff = unwrapped_angle - last_unwrapped_angle
                # 處理角度跳變
                if angle_diff > 90:
                    angle_diff -= 180
                elif angle_diff < -90:
        ...
        # PID 控制
        pid_voltage = PID_VOLTAGE_BASE
        pid_error = 0.0
        pid_output_raw = 0.0
        
        if ENABLE_PID and fg_controller.connected and tracker_state['recording'] and np.isfinite(ang_for_draw):
            if MODE.lower() == "straight":
                # Straight mode: 控制角度 → 0°
                pid_output_raw, p_term, i_term, d_term = pid_controller.compute(
                    PID_SETPOINT_STRAIGHT, ang_for_draw, timestamp
                )
                pid_voltage = PID_VOLTAGE_BASE + pid_output_raw
                pid_error = PID_SETPOINT_STRAIGHT - ang_for_draw
            else:
                # Rotation mode: 控制累積角度 → 90°
                pid_output_raw, p_term, i_term, d_term = pid_controller.compute(
                    PID_SETPOINT_ROTATION, cumulative_angle, timestamp
                )
                pid_voltage = PID_VOLTAGE_BASE + pid_output_raw
                pid_error = PID_SETPOINT_ROTATION - cumulative_angle
            
            # 更新函數產生器電壓
            fg_controller.set_voltage(pid_voltage)
        ...
        if MODE.lower() == "rotation":
            cv2.putText(display_frame, f"Cumul: {cumulative_angle:.2f} deg", (20, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,0,255), 2)
        else:
        ...
        # PID 資訊
        if ENABLE_PID and tracker_state['recording']:
            cv2.putText(display_frame, f"PID V: {pid_voltage:.3f}V | E: {pid_error:.2f}", (20, 130), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,0,255), 2)
        ...
        cv2.putText(display_frame, f"Cap: {info['capture_fps']:.1f} FPS | Proc: {info['process_fps']:.1f} FPS", 
                   (20, 190), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,255), 2)
        
        # 放入結果佇列
        result_data = {
            'frame': display_frame,
            'raw_frame': frame_rotated.copy(),
            'timestamp': timestamp,
            ...
            'speed': inst_speed,
            't_s': t_s,
            'pid_voltage': pid_voltage,
            'pid_error': pid_error,
            'pid_output': pid_output_raw,
...
def show_help():
    """顯示幫助信息"""
    print("\n" + "="*60)
    print("整合式攝影機追蹤與函數產生器控制系統 (多執行緒 + PID)")
    print("="*60)
    print("攝影機控制：")
    print("  [Space]     - 開始/停止錄製")
    print("  [ESC/Q]     - 退出程式")
...
def main():
    ...
    print(f"PID 控制: {'啟用' if ENABLE_PID else '停用'}")
    if ENABLE_PID:
        if MODE.lower() == "straight":
            print(f"  Kp={PID_KP_STRAIGHT}, Ki={PID_KI_STRAIGHT}, Kd={PID_KD_STRAIGHT}")
            print(f"  目標: 角度 = {PID_SETPOINT_STRAIGHT}°")
        else:
            print(f"  Kp={PID_KP_ROTATION}, Ki={PID_KI_ROTATION}, Kd={PID_KD_ROTATION}")
            print(f"  目標: 累積角度 = {PID_SETPOINT_ROTATION}°")
    
    # 初始化 PID 控制器
    if ENABLE_PID:
        if MODE.lower() == "straight":
            pid_controller = PIDController(
                PID_KP_STRAIGHT, PID_KI_STRAIGHT, PID_KD_STRAIGHT,
                -PID_VOLTAGE_BASE, PID_VOLTAGE_BASE
            )
        else:
            pid_controller = PIDController(
                PID_KP_ROTATION, PID_KI_ROTATION, PID_KD_ROTATION,
                -PID_VOLTAGE_BASE, PID_VOLTAGE_BASE
            )
    else:
        pid_controller = None
    ...
    running = [True]
    stats = Stats()
    tracker_state = {'recording': False, 'pid_params': {
        'kp': PID_KP_STRAIGHT if MODE.lower() == "straight" else PID_KP_ROTATION,
        'ki': PID_KI_STRAIGHT if MODE.lower() == "straight" else PID_KI_ROTATION,
        'kd': PID_KD_STRAIGHT if MODE.lower() == "straight" else PID_KD_ROTATION
    }}
    ...
    process_t = threading.Thread(target=process_thread, args=(frame_queue, result_queue, running, stats, kf, tracker_state, mm_per_px, fg_controller, pid_controller), daemon=True)
        ...
        while running[0]:
            try:
                ...
                if tracker_state['recording'] and RECORD_OUTPUT:
                    if writer is None:
                        ...
                        tracker_state['actual_fps'] = actual_fps
                        print(f"開始錄影: {output_dir}")
                        print(f"  PID 參數已包含在資料夾名稱中")
                    ...
                    # 記錄資料 (加入 PID 相關資料和 box)
                    rec_data.append((
                        data['frame_idx'], data['t_s'],
                        data['fx'], data['fy'],
                        data['fx_mm_abs'], data['fy_mm_abs'],
                        data['angle'], data.get('cumulative_angle', 0.0),
                        data.get('pid_voltage', PID_VOLTAGE_BASE),
                        data.get('pid_error', 0.0),
                        data.get('pid_output', 0.0),
                        mm_per_px,
                        data.get('box')  # 記錄 box
                    ))
            ...
            if key in (27, ord('q')):
                ...
                if not tracker_state['recording']:
                    ...
                    print("開始錄製...")
                    if fg_connected:
                        print("  請按 [1-4] 選擇函數產生器模式以開始 PID 控制")
                else:
                ...
                if fg_connected:
                    ...
                    success = fg_controller.switch_mode(mode_num)
                    if success:
                        selected_mode = mode_num
                        print(f"✓ 已選擇 Mode {mode_num}，現在可以按 [Space] 開始錄製")
                else:
                    print("函數產生器未連接")
...
def process_and_export_data(rec_data, output_dir, mm_per_px):
    ...
    # 分離 box 資料（PID 版本有13個欄位）
    box_rec = []
    data_without_box = []
    for row in rec_data:
        if len(row) >= 13:  # 包含 box
            frame_idx, t_s, fx, fy, fx_mm_abs, fy_mm_abs, angle, cumulative_angle, pid_v, pid_e, pid_o, mmpx, box = row
            box_rec.append((frame_idx, t_s, fx, fy, box))
            data_without_box.append((frame_idx, t_s, fx, fy, fx_mm_abs, fy_mm_abs, angle, cumulative_angle, pid_v, pid_e, pid_o, mmpx))
        else:
            data_without_box.append(row)
    
    # 整理資料
    df = pd.DataFrame(data_without_box, columns=[
        "frame","t_s","x_px_filt","y_px_filt","x_mm_abs","y_mm_abs","angle_deg_raw","cumulative_angle",
        "pid_voltage","pid_error","pid_output","mm_per_px"
    ])
