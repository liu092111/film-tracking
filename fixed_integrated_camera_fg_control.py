...
WINDOW_TITLE      = "Integrated Camera & Function Generator Control - FPS Fixed"
...
class CameraTracker:
    ...
    def __init__(self, fg_controller):
        ...
        # FPS 修正相關變數
        self.start_time = None  # 錄製開始的實際時間
        self.actual_fps_buffer = deque(maxlen=60)  # 保存最近60幀的FPS計算
    ...
    def process_frame(self, frame):
        ...
        if self.state == 1 and self.start_time is not None:  # RECORDING
            t_s = current_time - self.start_time  # 實際錄製時間
        else:
            # 預覽模式仍可使用理論計算或實際時間
        ...
        if self.state == 0:  # PREVIEW
            # 旋轉後畫面尺寸變化，調整文字位置
            cv2.putText(display_frame, "FPS FIXED - [SPACE] Start Recording, [1-4] FG Mode, [0] FG Off",
        ...
        if self.state == 1 and RECORD_OUTPUT:  # RECORD
            ...
            if self.writer is None:
                ...
                if len(self.frame_buf) >= 10:  # 減少暖機幀數，使用實際FPS
                    # 【關鍵修正】使用實際測量的FPS
                    fps_out = actual_fps
                    
                    run_tag = datetime.now().strftime("%Y%m%d_%H%M%S")
                    self.output_dir = f"{run_tag}_{MODE}_integrated_fixed"
...
def show_help():
    """顯示幫助信息"""
    print("\n" + "="*60)
    print("整合式攝影機追蹤與函數產生器控制系統 - FPS修正版")
    print("="*60)
    print("修正項目：")
...
def process_and_export_data(camera_tracker):
    ...
    if camera_tracker.output_dir and os.path.exists(camera_tracker.output_dir):
        ...
        run_tag = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_dir = f"{run_tag}_{MODE}_integrated_fixed"
    ...
    if mask_ang.any():
        ...
        if MODE.lower() == "straight":
            ...
            ang_unwrap[mask_ang] = angle_deviations
        else:
            # 旋轉模式使用正常的角度展開
            ang_unwrap[mask_ang] = unwrap_angles_deg(valid_angles)
    ...
    # 【新增】建立GIF檔案
    gif_path = None
    if EXPORT_GIF and camera_tracker.out_path and os.path.exists(camera_tracker.out_path):
        gif_path = os.path.join(output_dir, f"camera_{MODE}_tracked.gif")
        create_gif_from_mp4(camera_tracker.out_path, gif_path)
